/****************************************************************
Main CPP for PhysBot. This file handles the pin change interrupts
...
Modification to keep consistent with other changes in files. This file
is largely unchanged from the original. Many of the lowest level access
routines and interrupt service handlers were left pretty much, as is.
Higher level routines were modified to match our specific needs and
hardware.
 ...
 */
volatile char lastPC0PinState = 0x0E;  // For pins 9, 10, 11, PB1-3
volatile char lastPC1PinState = 0x3F;  // For pins A0-A5/14-19, PC0-5
volatile char lastPC2PinState = 0x08;  // For pin 3, PD3
...
//#define WHISKER_HIGH_DELAY   0 // UNUSED?

// Initialize the Mask values. These are set in setPinChangeInterrupt()
char PBMask = 0;
char PCMask = 0;
char PDMask = 0;

volatile int pinFunction[10];     // Store the currently assigned function
                                   //  of the PCINT associated with each pin
                                   //  in this array. Array indices are of
                                   //  the type "PCINT_pinname".

// These are currently unused
//extern void (*whiskerAction[10])(void); // Declared in PhysBotBumper.cpp
//PhysBotSoftwareSerial *RBSPObject=0;

/** Interrupt Service Routine is the action that is taken when an interrupt
 * (read: some external event like a pin voltage change) asserts that something
 * must be done. Because everything else in the processor is halted we must do
 * whatever we need to do quickly and get out fast.
 */


/** The PhysBot uses pin change interrupts for detecting wheel encoder ticks and
...
ISR(PCINT0_vect)
{
   ...
   *  are all high.
   */
  char PBTemp = PINB & PBMask;  // Capture the state of the pins-of-interest now,
                                //  before they have a chance to change.
  PC0Handler(PBTemp);
}

void
PC0Handler(char PBTemp)
{
...
ISR(PCINT1_vect)
{
  ...
  char PCTemp = PINC & PCMask;  // Capture the state of the pins-of-interest now,
                               //  before they have a chance to change.
...
ISR(PCINT2_vect)
{
  ...
  char PDTemp = PIND & PDMask;// Capture the state of the pin-of-interest now,
                               //  before they have a chance to change.
  PC2Handler(PDTemp);
}

void
PC2Handler(char PDTemp)
{
...
/** The GCT for all the ISRs once the pins and actions have been sorted.
 * @param {int} pinIndex is the index into the array which holds the thing we
 * 					are supposed to do.
 */
void
pinFunctionHandler(int pinIndex)
{
  switch(pinFunction[pinIndex])
  {
    case LENCODER:
	  if (lastLRise + ENC_HIGH_DELAY < micros())
	  		PhysBotEncoders::encoderObj->wheelTick(LEFT);
      break;
    case RENCODER:
      if (lastRRise + ENC_HIGH_DELAY < micros())
      		PhysBotEncoders::encoderObj->wheelTick(RIGHT);
      break;
    case WHISKER:
...
/** set the control values for the ISR routines based on the pin and the
 * pin's role.  Mike Hord's work.
 */
void
setPinChangeInterrupt(int pin, int role)
{
  switch(pin)
  {
