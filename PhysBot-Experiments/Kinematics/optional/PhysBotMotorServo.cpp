...
PhysBotMotorServo::update()
{
	...
	if ( kinematicsObject == 0 ) return;	// no pointer available to kinematicsControl

	long tickCount = kinematicsObject->getTicks(BOTH);
	...
	// Servo handler
	float instVel = kinematicsObject->intstantaneousSpeed( BOTH );
	...
	float here = kinematicsObject->distanceTraveled( BOTH );

// The following is for diagnostics.
//	Serial << "{" << kinematicsObject->getTime(BOTH) << ", " << here << ", " << instVel << "}," << endl; // DEBUG
